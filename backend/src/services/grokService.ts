...
/**
 * Detects which AI provider to use based on the API key prefix.
 * - gsk_... => Groq API  (groq.com) — uses llama/mixtral models
 * - xai-... => xAI/Grok API (x.ai) — uses grok-* models
 * - sk-...  => OpenAI API (openai.com) — uses gpt-* models
 */
...
const buildRequestBody = (model: string, prompt: string, supportsJsonMode: boolean) => {
  const body: any = {
    model,
    messages: [
      {
 ...
 */
export const queryGrokAI = async (query: string): Promise<GrokAIResponse> => {
  const normalized = normalizeQuery(query);

  // Feature 12: Check AI Cache first
  const cached = await AICache.findOne({ query: normalized });
  if (cached) {
    console.log(`[AI] Cache hit for query: "${normalized}"`);
    return cached.response as GrokAIResponse;
  }
 ...
 */
async function handleAIResponse(normalizedQuery: string, response: Response): Promise<GrokAIResponse> {
  ...
  // Cache the result
  try {
    await AICache.create({ query: normalizedQuery, response: result });
